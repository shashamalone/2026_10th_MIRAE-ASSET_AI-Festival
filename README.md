...
현재 데이터 정본은 주최측이 2026-08-24 제공한 정상 XLSX 8개로 구성된
`financial-products-2026-08-24`이며, `__MACOSX/._*`는 입력에서 제외한다.
운영 경로는 PostgreSQL 17 + pgvector + Oxigraph + 읽기전용 FastAPI다.
`../data/data`의 2026-07-11 CSV·파생·관계 데이터는 legacy 참고자료이며,
공식 원문·sidecar·cutoff·현행 상품 ID를 재검증한 관계 근거만 선택적으로 사용한다.
