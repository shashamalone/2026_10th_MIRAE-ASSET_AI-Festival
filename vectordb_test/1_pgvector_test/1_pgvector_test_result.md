...
```
vector   0.8.1
pg_trgm  1.6      (한국어 부분어 폴백 검증용, contrib)
```
...
| # | 테스트 | 내용 | exit | 판정 |
|---|---|---|---|---|
| 1 | `test_connection.py` | 접속 · 서버 버전 · `vector`/`pg_trgm` 확장 · text search 설정 목록 | 0 | **PASS** |
...
단, PASS 는 **"pgvector 로 이전 가능"** 이라는 뜻이지 **"한국어 FTS 가 쓸 만하다"** 는 뜻이 아니다.
