...
CSV_INPUTS = sorted((ROOT / "data/csv").glob("*_master_20260711.csv")) + \
             sorted((ROOT / "data/enriched").glob("*.csv")) + \
...
SQL = {
1: """SELECT b.pd_no,b.pd_nm,b.pd_pbcm,b.crd_grd,b.srfc_irt,b.mat_dt,b.buy_yield,b.buyable_quantity,b.pd_std_info_update FROM raw.bond_kr_master b WHERE b.pd_nm='에스케이하이닉스224-2'""",
2: """SELECT b.pd_no,b.pd_nm,b.isu_dt,b.mat_dt,e.remaining_days,b.srfc_irt,b.after_tax_yield,b.pd_std_info_update FROM raw.bond_kr_master b JOIN enriched.bond_kr_enriched e USING(pd_no) WHERE b.pd_nm='개인투자용국채 02480-3006'""",
3: """SELECT b.pd_no,b.pd_nm,b.bd_knd,b.pd_pbcm,b.crd_grd,e.crd_grd_norm,e.maturity_bucket,b.dur FROM raw.bond_kr_master b JOIN enriched.bond_kr_enriched e USING(pd_no) WHERE b.pd_nm='현대해상화재보험7(후)(콜/후)'""",
...
8: """SELECT f.itm_no,f.itm_nm,f.or_attr_desc,f.fd_nast_suma,f.fd_mm1_ern_r,f.fd_mm3_ern_r,f.fd_mm6_ern_r,f.fd_yr1_ern_r,f.zrin_fd_ivst_risk_grd_nm,f.sale_yn FROM enriched.fund_pub_dedup f WHERE f.itm_nm='미래에셋코어테크증권자투자신탁(주식) 종류A'""",
9: """SELECT f.itm_no,f.itm_nm,f.bmrk_nm,f.curr_cd,f.fd_ivst_rgn_desc,f.pers_corp_desc,f.zrin_fd_ivst_risk_grd_nm,f.fd_nast_suma FROM enriched.fund_pub_dedup f WHERE f.itm_nm='삼성 베스트 MMF 법인 제1호'""",
10: """SELECT f.itm_no,f.itm_nm,f.mtco_itm_no,f.rptt_ksd_itm_no,f.sale_yn,f.fd_nast_suma,f.fd_yr1_ern_r FROM enriched.fund_pub_dedup f WHERE f.itm_nm IN ('우리반도체BIG2플러스증권투자신탁(채권혼합)ClassC-P','우리반도체BIG2플러스증권투자신탁(채권혼합)ClassC-Pe') ORDER BY f.itm_no""",
11: """SELECT b.pd_no,b.pd_nm,b.pd_pbcm,b.crd_grd,b.mat_dt,b.buy_yield,b.buyable_quantity,b.pd_std_info_update FROM raw.bond_kr_master b JOIN enriched.bond_kr_enriched e USING(pd_no) WHERE b.curr_cd='KRW' AND b.buyable_quantity>0 AND e.crd_grd_rank<=4 ORDER BY b.buy_yield DESC NULLS LAST,b.pd_no""",
...
13: """SELECT b.pd_no,b.pd_nm,b.crd_grd,e.crd_grd_rank,e.remaining_days,b.buy_yield,b.buyable_quantity FROM raw.bond_kr_master b JOIN enriched.bond_kr_enriched e USING(pd_no) WHERE b.std_pd_mcls_nm='회사채' AND b.buyable_quantity>0 AND e.crd_grd_rank<=4 AND e.remaining_days<=1095 ORDER BY b.buy_yield DESC NULLS LAST,b.pd_no LIMIT 10""",
...
19: """SELECT f.itm_no,f.itm_nm,f.fd_yr1_ern_r,f.fd_nast_suma,f.exchdg_yn,f.fd_ivst_rgn_desc,f.zrin_fd_ivst_risk_grd_nm FROM enriched.fund_pub_dedup f WHERE f.prvo_pbff_desc='공모' AND f.sale_yn='판매중' AND f.fd_yr1_ern_r>0 AND f.fd_nast_suma>=100000000000 ORDER BY f.fd_yr1_ern_r DESC,f.itm_no""",
20: """SELECT 'ETF' AS product_type,k.pd_itm_no AS product_id,k.pd_nm AS product_name,k.du_last_aum AS aum,k.du_er_1y AS return_1y,t.theme AS evidence FROM raw.etf_kr_master k JOIN relations.etf_theme t USING(pd_itm_no) WHERE k.pd_grp_no='ETF' AND t.theme ILIKE '%반도체%' UNION ALL SELECT 'FUND',f.itm_no,f.itm_nm,f.fd_nast_suma,f.fd_yr1_ern_r,COALESCE(f.bmrk_nm,f.or_attr_desc) FROM enriched.fund_pub_dedup f WHERE f.prvo_pbff_desc='공모' AND (f.bmrk_nm ILIKE '%반도체%' OR f.or_attr_desc ILIKE '%반도체%') ORDER BY aum DESC NULLS LAST,product_id""",
...
25: """SELECT f.itm_no,f.itm_nm,f.or_co_xtn_itt_cd,f.or_attr_desc,f.fd_nast_suma,f.sale_yn FROM enriched.fund_pub_dedup f WHERE f.itm_nm ILIKE '%국민성장%' ORDER BY f.itm_no""",
26: """SELECT 'BOND' AS product_type,b.pd_no AS product_id,b.pd_nm AS product_name,b.crd_grd AS rating,b.buy_yield AS yield_value,b.buyable_quantity AS quantity,NULL::numeric AS weight,NULL::numeric AS aum FROM raw.bond_kr_master b WHERE b.pd_pbcm='에스케이하이닉스(주)' AND b.buyable_quantity>0 UNION ALL SELECT 'ETF',k.pd_itm_no,k.pd_nm,NULL,NULL,NULL,h.weight,k.du_last_aum FROM relations.etf_holding h JOIN raw.etf_kr_master k USING(pd_itm_no) WHERE h.holding_code_raw='000660' AND k.pd_grp_no='ETF' ORDER BY aum DESC NULLS LAST,product_id""",
...
def make_gold(catalog: dict, terms: dict[str, dict]) -> tuple[list[dict], list[dict], list[dict]]:
    source = ROOT / "expected_question/2026_expected_queries.csv"
    ...
    meta = {"data_cutoff": "2026-07-11", "expected_queries_sha256": sha(source),
...
TABLE_CODE = {
    ...
    "PRFD01N001": ["enriched.fund_pub_dedup", "raw.fund_pub_master"],
    "derived:bond_kr_enriched": ["enriched.bond_kr_enriched"],
    "derived:etf_kr_enriched": ["enriched.etf_kr_enriched"],
    "derived:fund_pub_dedup": ["enriched.fund_pub_dedup"],
...
RULES = {
    ...
    "fp:netAssets": {"value_rule": "펀드 집계는 fund_pub_dedup만 사용"},
}
...
def make_contexts(catalog: dict, nl: list[dict], routing: list[dict], terms: dict[str, dict]) -> None:
    ...
    for qid, gold in by_q.items():
        ...
        for term in selected:
            ...
            citems.append(item)
        cq[qid] = citems
    meta = {"data_cutoff": "2026-07-11", "catalog_sha256": catalog["catalog_sha256"],
    ...
    c = {"condition": "C", "meta": meta, "physical_schema": catalog["tables"],
         ...
         "global_rules": ["data cutoff <= 2026-07-11", "주최측 값 우선",
