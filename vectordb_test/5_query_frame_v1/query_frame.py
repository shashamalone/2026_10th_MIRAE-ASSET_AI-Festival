# -*- coding: utf-8 -*-
"""1단계 Query Understanding — Semantic Query Frame v1 (실험).

여기서 35문항으로 검증한 뒤 확정된 것만 src/agent/ 로 올린다.

자연어 질의를 SQL/Graph/Vector 가 나중에 조합할 수 있는 의미 요소로 쪼갠다.
...
SCHEMA_VERSION = "v1-draft"     # 35문항 검증 통과 후 "v1" 로 동결한다
MODEL = "HCX-007"               # --model 로 덮어쓴다. 최종 선택은 실측으로 정한다
...
future_value          요구한 값의 시점이 기준일 2026-07-11 보다 뒤다. 아직 실현되지 않은 실적·확정치.
                      질문에 2026-07-11 이 적혀 있는 것은 미래가 아니다. 그건 기준 시점을 못박은
...
if __name__ == "__main__":
    # 자기검사 — API 없이 도는 것만. guard 가 실제로 덮는지, to_conditions 가 의미를 보존하는지.
    g = guard({"task": "용어정의", "domain_candidates": ["bond_kr", "BOND"],
               "constraints": [{"raw": "AA- 이상", "field_text": "신용등급", "operator": "이상",
                                "value_text": "AA-", "kind": "categorical",
               ...
               "limit": "10"})
    assert g["domain_candidates"] == ["bond_kr"], g["domain_candidates"]
    assert g["constraints"][0]["operator"] == ">=", "이상 → >= 로 정규화돼야 한다"
    assert g["constraints"][1]["grounding_status"] == "unresolved", "qualitative 는 resolved 일 수 없다"
    assert g["ordering"][0]["direction"] == "desc"
    assert g["limit"] == 10 and isinstance(g["limit"], int)
    assert g["task"] == "filter_rank", f"entities 없는 lookup 은 기각돼야 한다: {g['task']}"
    assert set(FIELDS) <= set(g), "guard 는 14필드를 모두 채운다"
    assert len(g["_guard"]) >= 5, g["_guard"]

    assert leaks({"requested_fields": [{"text": "crd_grd_rank"}]}) == ["crd_grd_rank"]
    assert leaks({"targets": [{"text": "회사채"}]}) == []
    assert leaks({"constraints": [{"field_text": "fp:ratingRank"}]}) == ["fp:r"]
    assert guard({})["temporal"]["kind"] == "latest_snapshot"

    conds = to_conditions(g)
    assert any("신용등급 >= AA-" in c for c in conds), conds
    assert any("기준 미확정" in c for c in conds), conds
    print(f"query_frame 자기검사 PASS — 14필드, guard 보정 {len(g['_guard'])}건")
    for c in conds:
        print("   ", c)
