...
| 값의 모양 | 저장 위치 | 예시 |
|---|---|---|
| 상품 1개당 값 1개 (스칼라) | RDB 보강 테이블 (`data/enriched/`) | ter(총보수), replication, hedge_type, is_sellable, 등급 ordinal |
...
```
data/csv/        원본 변환본. 동결 — 컬럼 추가·값 수정 금지 (주최측 데이터 우선 규칙의 뿌리)
data/enriched/   원본 PK + 보강 스칼라 컬럼 + 출처 컬럼(*_source)
data/relations/  롱포맷 관계 테이블 (주어ID, 목적어, source)
data/external/   외부 수집 원천 (EXTERNAL_DATA_PLAN.md 규칙: as_of 2026-07-11 이전, 사이드카 JSON)
ontology/*.ttl   relations + enriched 스키마에서 생성
```

## 생성 규칙

1. **원본 동결**: `data/csv/*_master_*.csv`는 절대 수정하지 않는다. 깨진 행(펀드 itm_no=`"`) 배제도 파생 테이블에서만.
2. **출처 컬럼 필수**: 보강 값에는 `{컬럼}_source`를 붙인다 (`RDB` = 주최측 값, `LSEG` 등 = 외부). 답변 evidence가 이 컬럼을 그대로 인용한다.
...
| 파일 | 내용 | 스크립트 |
|---|---|---|
| `data/csv/PRFD01N001_fund_pub_dedup_20260711.csv` | 펀드 1행화(11,138), 속성코드는 `prfd_attr_cds`로 집약 | `build_fund_dedup.py` |
...
| `data/relations/etf_theme.csv` | 국내ETF↔테마 (LSEG themes, 176종) 롱포맷 | `build_etf_enrichment.py` |
