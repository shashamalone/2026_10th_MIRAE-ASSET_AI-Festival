...
SRC = ROOT / "src"
OUT = Path(__file__).parent / "Gragh-test" / "graph.ipynb"
...
FILES = [
    ...
    ("tools.graph_schema", "tools/graph_schema.py"),
    ("tools.graph_entity", "tools/graph_entity.py"),
    ("tools.graph_plan", "tools/graph_plan.py"),
    ("agent.text2sparql", "agent/text2sparql.py"),
    ...
    # ("kb.build_graph_schema_index", "kb/build_graph_schema_index.py"),
...
TEST_CELLS = [
    ("md", "# 노드별 테스트\n\n"
           ...
           "select_route → execute_rdb 또는 execute_graph(route 에 따라) → verify_results → render_answer"),
    ...
    ("code", 'out = N.execute_graph(state); state.update(out); out\n'
             '# route 가 graph_only / graph_then_rdb 일 때만 의미 있음'),
...
cells = [
    md("# src 통합 노트북\n\n"
       ...
       "재생성: `python test/build_notebook.py` (src 최신본 기준으로 덮어씀 — 노트북에서 수정 중이면 먼저 sync 할 것)"),
    code(SETUP),
]
