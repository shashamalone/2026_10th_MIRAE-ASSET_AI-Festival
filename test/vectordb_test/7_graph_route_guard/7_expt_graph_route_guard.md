...
- `data/csv`와 외부 원천은 수정하지 않는다. `common.ttl`에는
`SubsidiaryRelation → supportedBy → Document` domain 계약만 추가한다.
...
- UPDATE/SERVICE 거부
- Holding `47,016/47,016`, cutoff 이내 SubsidiaryRelation 전체의 `supportedBy`
문서 evidence coverage `100%`
- 문서명·발행기관·발행일·구조화 원문 발췌 필수값 coverage `100%`
- cutoff 이후 관계의 evidence link `0건`
- 위 조건이 충족되기 전에는 GRAPH_ONLY 운영 승격 금지
