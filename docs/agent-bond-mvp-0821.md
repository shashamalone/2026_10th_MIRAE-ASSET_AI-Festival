...
| 검색 방식 | cosine 유사도 (L2 정규화 후 `IndexFlatIP`) | `kb/build_bond_index.py` |
| 점수 하한 | `0.45` 미만은 버림 → 노이즈 **4건 → 0건** | §3-2 (1) |
| `artifacts/bond.faiss` | 520 KB | |
| `artifacts/bond_terms.json` | 78 KB | |
...
| # | 완료조건 | 판정 | 근거 |
|---|---|:---:|---|
| 1 | 채권 TTL에서 `rdfs:label`·`rdfs:comment`를 읽을 수 있다 | **PASS** | `kb/build_bond_index.py:collect()`. 한국어 리터럴 우선(`pick()`) |
...
| 4 | `artifacts/bond.faiss`가 생성된다 | **PASS** | 520 KB, `ntotal=130` |
| 5 | `artifacts/bond_terms.json`이 생성된다 | **PASS** | 78 KB, 배열 index == `vector_id` |
| 6 | `bond_schema_search()`가 Top-K를 반환한다 | **PASS** | `tools/bond_schema.py`. 단 하한 미달은 K를 억지로 채우지 않는다 |
| 7 | `classify_intent`가 `BOND / SCHEMA_SEARCH`를 반환할 수 있다 | **PASS** | "반환할 수 있다"는 충족(4건 중 2건). **다만 항상 그렇지는 않다** → §6-2 |
| 8 | LangGraph가 `classify_intent → search_bond_schema → answer` 순서로 실행된다 | **PASS** | `agent/agent_core.py`. 분기 없는 직선 |
...
> `0.45`는 130건 규모에서 실측으로 고른 값이다. 인덱스가 커지거나 임베딩 모델이 바뀌면 다시 재봐야 한다. `config.py` 한 줄이라 조정은 쉽다.

#### (2) 임베딩 캐시 + 429 백오프 (`clova.py:embed_many`)
...
`kb/build_bond_index.py:53-55`가 `skos:altLabel`을 검색 텍스트에 붙인다. 의도는 사용자가 쓰는 말과 스키마 라벨이 다른 경우를 잡는 것이다.
...
**결과:** 최상위 `clova.py`로 승격하고 `agent/`·`tools/`·`kb/` 전부가 이것 하나를 쓴다. `script/test_clova.py`는 규격 탐색 기록으로 그대로 둔다.
...
├── kb/
│   └── build_bond_index.py            TTL → FAISS 인덱스 빌더 (오프라인, 1회)
...
> **`artifacts/`가 `.gitignore`에 있는 이유:** 세 파일 모두 `ontology/*.ttl`에서 **재생성 가능한 산출물**이기 때문이다. 커밋하면 TTL을 고칠 때마다 1.5 MB짜리 바이너리 diff가 따라붙는다. 새로 받은 사람은 `python3 kb/build_bond_index.py` 한 번을 돌리면 된다(§7).
...
- **핵심 상수:** `BOND_TTL_PATHS`(리스트 — §3-1 (2)) · `BOND_INDEX_PATH` · `BOND_TERMS_PATH` · `BOND_TOP_K=5` · `BOND_SCORE_FLOOR=0.45` · `EMBEDDING_MODEL="bge-m3"` · `INTENT_MODEL="HCX-DASH-002"` · `ANSWER_MODEL="HCX-005"` · `CLOVA_HOST`
- **주의점:** `BOND_TTL_PATHS`나 `EMBEDDING_MODEL`을 바꾸면 **인덱스를 반드시 재빌드**해야 한다. 안 하면 `bond.faiss`와 `bond_terms.json`이 어긋나고, `tools/bond_schema.py:_load()`가 개수 불일치를 잡아 `SystemExit`로 죽는다(조용히 틀린 답을 내놓는 것보다 낫다).
...
### `kb/build_bond_index.py` — 85행

- **한 줄 역할:** 온톨로지 TTL을 읽어 **검색용 벡터 인덱스를 만드는 오프라인 빌더.** 평소엔 안 돌고, TTL을 고쳤을 때만 돌린다.
- **입력 → 출력:** `ontology/bond_kr.ttl` + `ontology/common.ttl` → `artifacts/bond.faiss` + `artifacts/bond_terms.json`
- **핵심 함수:**
  - `pick(g, s, prop)` — 같은 프로퍼티에 `@ko`/`@en`이 둘 다 있으면 **한국어 우선**. 질문이 한국어라 한국어 라벨이 붙어야 검색이 맞는다
  - `collect()` — 주석 있는 `fp:` resource를 **URI 정렬 순서로** 모은다. 이 순서가 곧 `vector_id`다
  - `main()` — `embed_many` → `float32` → `normalize_L2` → `IndexFlatIP` → 저장
- **주의점:**
  - **`rdfs:comment`가 없는 resource는 인덱스에 넣지 않는다.** 이 인덱스의 목적은 "안전한 채권" 같은 말을 `fp:` 용어로 번역하는 것이고, 번역 근거가 되는 게 주석이기 때문이다. 라벨만 있는 코드리스트 개체 37개가 이 규칙으로 빠진다(§3-2 (3)).
  - **정렬이 핵심이다.** `sorted(subs, key=str)` 덕에 재빌드해도 `vector_id`가 흔들리지 않는다. 이걸 빼면 TTL을 한 줄만 고쳐도 전체 매핑이 뒤섞인다.
  - 검색 단위는 **resource 1개 = vector 1개**. TTL을 일반 문서처럼 chunking하지 않는다.
  - `IndexFlatIP`는 전수 비교다. 130건에서는 즉시지만 **수만 건이 되면 `IndexIVFFlat` 등으로 바꿔야** 한다.

### `tools/bond_schema.py` — 50행
...
- **핵심 함수:**
  - `_load()` — `@lru_cache(maxsize=1)`. FAISS 인덱스와 term JSON을 **프로세스당 한 번만** 읽는다
  - `bond_schema_search(text, k, floor)` — 임베딩 → 정규화 → FAISS 검색 → 하한 필터 → term 결합
- **주의점:**
  - **캐싱이 성능상 필수다.** 매 검색마다 다시 읽으면 질의 하나에 수백 ms가 붙어 응답 예산(60초 권장, 15초 목표)을 갉아먹는다.
  - `_load()`가 **인덱스 개수와 term 개수 불일치를 검사**해 다르면 `SystemExit`. 어긋난 채로 돌면 엉뚱한 용어를 근거라며 내놓는다.
  - **하한 미달이면 K를 채우지 않는다.** 반환 개수가 0~K 사이로 변하니 호출부는 빈 리스트를 반드시 처리해야 한다(`agent/nodes.py:answer`가 처리한다).
...
- **주의점:**
  - **`APP`이 import 시점에 만들어진다.** 그래프 컴파일은 가볍지만, `import agent.agent_core` 자체가 `tools.bond_schema` → `faiss`·`clova`를 끌어온다. 인덱스 파일 읽기는 첫 검색까지 지연된다(`_load()`가 `lru_cache`라).
...
**CLOVA 콘솔이 주는 샘플 코드를 그대로 쓰면 동작하지 않는다.** 세 군데가 다르고, 셋 다 실측으로 알아낸 것이다. 이어받는 사람이 같은 곳에서 시간을 쓰지 않도록 적는다. 구현은 전부 `clova.py:chat()`에 있다.
...
**이미 해본 것:** `ANSWER_SYSTEM`에 *"네가 알고 있는 금융 일반 지식은 쓰지 마라. comment에 없으면 없는 것이다"* 를 넣고, 심지어 **이 듀레이션 사례를 예시로 명시**했다(`agent/nodes.py:26`). `HCX-005`가 지키지 않는다.
...
resource 130개 수집 (bond_kr.ttl, common.ttl)
  ...
  bond.faiss           130벡터 × 1024차원  520KB
  bond_terms.json      78KB
소요 ...s
```
