...
| 항목 | 값 |
|---|---|
| 주최측 원본 XLSX(현재 위치) | `data/data/csv/pref01n001_data.xlsx` |
...
원본 XLSX 해시는 운영 `/db/version`에 기록된 `PREF01N001.data_sha256`과
일치한다. 즉 현재 `data/data/csv`에 놓인 파일은 이름상 legacy 디렉터리에
있지만 내용은 서버에 적재된 주최측 8/24 원본과 동일하다. 수집기는 원본을
수정하지 않고, 이 파일을 정규화해 보존한 읽기 전용 CSV를 상품 식별에 쓴다.
...
원격 stage/cutover 결과는 완료 후 이전/신규 volume, public Graph triple 수와
receipt 경로를 이 문서에 추가한다. 배포 archive SHA-256은
