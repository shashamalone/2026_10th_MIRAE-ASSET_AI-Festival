...
def build(args: argparse.Namespace) -> dict:
    ...
    for row in relation.itertuples(index=False):
        ...
        if row.pd_itm_no not in document_nodes:
            ...
            publisher = entry.get("source") or entry.get("document")
            if not publisher:
                raise ValueError(
                    f"provenance publisher missing for product {row.pd_itm_no}"
                )
            ...
            etf_graph.add((document, FP.documentPublisher, Literal(publisher)))
            etf_graph.add(
