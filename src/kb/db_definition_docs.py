...
from kb.catalog import VIEW_DEFINITIONS, TableDef
from kb.manifest import (
    DATASET_VERSION,
    EXTERNAL_CUTOFF,
    RELEASE_DATE,
    ROOT,
...
def automatic_header(title: str, source_text: str) -> list[str]:
    return [
        ...
        "자동 생성 파일이므로 직접 편집하지 말고 `python src/kb/build_catalog.py`를 실행합니다.",
        source_text,
        "",
...
def rdb_definition_markdown(inspections, catalog: tuple[TableDef, ...]) -> str:
    ...
    sections = automatic_header(
        "RDB DEFINITION V2.0",
        "물리 정의의 정본은 [단일 카탈로그](../../src/kb/catalog.py)와 "
...
def vector_definition_markdown(catalog: tuple[TableDef, ...]) -> str:
    ...
    sections = automatic_header(
        ...
        "물리 정의의 정본은 [단일 카탈로그](../../src/kb/catalog.py), "
        "[Vector DDL](../../sql/v2/001_platform_schema.sql), "
        "[임베딩 빌더](../../src/kb/build_vectors.py)입니다.",
    )
    sections.extend(
        [
            "## 이 문서를 읽는 순서",
...
def graph_definition_markdown() -> str:
    ...
    sections = automatic_header(
        ...
        "[Graph 인스턴스 빌더](../../src/kb/build_graph_instances.py)가 소유합니다.",
    )
    sections.extend(
        [
            "## 이 문서를 읽는 순서",
