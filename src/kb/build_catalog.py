...
from kb.catalog import VIEW_DEFINITIONS, TableDef, build_catalog  # noqa: E402
from kb.db_definition_docs import (  # noqa: E402
    graph_definition_markdown,
    rdb_definition_markdown,
    vector_definition_markdown,
)
from kb.manifest import (  # noqa: E402
    DATASET_VERSION,
    EXTERNAL_CUTOFF,
    RELEASE_DATE,
    ROOT,
...
자동 생성 파일입니다. 직접 편집하지 말고 `python src/kb/build_catalog.py`를 실행합니다.
...
def definition_markdown(catalog: tuple[TableDef, ...]) -> str:
    sections = [
        ...
        "",
        "자동 생성 파일입니다. 모든 물리 컬럼은 공식 XLSX 스키마 또는 `src/kb/catalog.py`의 단일 카탈로그에서 생성됩니다.",
        "",
    ]
    for table in catalog:
        pk = ", ".join(
