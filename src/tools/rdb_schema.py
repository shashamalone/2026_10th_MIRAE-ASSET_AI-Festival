...
DOMAIN_SQL_CAVEATS: dict[str, list[str]] = {
    "채권": [
        ...
        "상품명으로 특정 종목을 찾을 때는 양쪽의 공백을 지우고 비교한다: "
        "REPLACE(pd_nm, ' ', '') LIKE '%공백을제거한키워드%'. 원본 표기의 띄어쓰기가 "
        "질문과 어긋나 정작 찾는 종목만 빠지는 사고가 실측됐다. 다만 아래 ESG채권 "
    ...
    "국내ETF": [
        ...
        "상품명으로 특정 종목을 찾을 때는 컬럼과 키워드 **양쪽 모두**에서 모든 공백을 "
        "지우고 비교한다. 반드시 REPLACE를 쓴다. "
        "맞는 형태: REPLACE(pd_nm, ' ', '') LIKE '%KODEX200%' "
        "틀린 형태: TRIM(pd_nm) LIKE '%KODEX 200%' (TRIM은 양끝 공백만 지워서 "
        "소용없고, 키워드에 공백을 남기면 매칭이 깨진다 - 둘 다 실측 실패했다). "
        "질문이 'KODEX 200'이면 키워드를 'KODEX200'으로 바꿔서 넣어야 한다. "
        "이 규칙을 어기면 원본 표기가 붙여쓰기인 종목이 결과에서 통째로 빠진다 "
    ...
    "해외ETF": [
        ...
        "상품명으로 특정 종목을 찾을 때는 양쪽의 공백을 지우고 비교한다: "
        "REPLACE(pd_nm, ' ', '') LIKE '%공백을제거한키워드%'. 원본 표기의 띄어쓰기가 "
        "질문과 어긋나 정작 찾는 종목만 빠지는 사고가 국내ETF에서 실측됐다.",
    ],
    "펀드": [
        ...
        "상품명으로 특정 종목을 찾을 때는 양쪽의 공백을 지우고 비교한다: "
        "REPLACE(itm_nm, ' ', '') LIKE '%공백을제거한키워드%'. 이 도메인은 위험등급 표기부터 "
        "공백이 섞여 있어 이름 계열 매칭도 같은 위험이 있다.",
    ],
}
