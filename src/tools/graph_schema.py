...
팀원의 gragh-test 노트북 `tools/graph_schema.py`(셀 33)를 이식했다. 로직은
그대로이고, import 경로만 우리 평면 구조(`config`, `graph_ids`)로 바꿨다.

벡터 인덱스는 재생성 가능한 ``artifacts/graph_schema_index.json``에 둔다.
이 워크스페이스엔 그 인덱스 파일이 없으므로 `_vector_hits`는 항상 None을
반환하고, `select_fragment`는 항상 lexical fallback(`_lexical_hits`)을 쓴다
- 반환값의 `selection_mode`에 "lexical_fallback"으로 남아 이 사실이 조용히
가려지지 않는다. 인덱스를 나중에 만들면 자동으로 임베딩 경로를 탄다.
"""
...
from agent.graph_logic.graph_ids import normalize_text
...
class SchemaCatalog:
    ...
    def load(cls) -> "SchemaCatalog":
        ...
        for name in TBOX_FILES:
            graph.parse((ONTOLOGY_DIR / name).as_posix(), format="turtle")
        return cls(graph)
    ...
    def _vector_hits(self, question: str, k: int) -> list[dict] | None:
        ...
        if payload.get("tbox_fingerprint") != self.fingerprint():
            return None
        # 이 워크스페이스엔 임베딩 클라이언트가 따로 없다 - 인덱스 파일이
        # 실제로 존재할 때만 이 지점에 도달하므로(위 file check), 지금은
        # 항상 위에서 None으로 빠진다. 인덱스를 만들 때 여기에 실제
        # 임베딩 클라이언트 호출을 연결하면 된다.
        raise NotImplementedError("임베딩 인덱스는 있지만 임베딩 클라이언트가 아직 연결되지 않았습니다")
